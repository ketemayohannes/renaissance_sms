<x-teacher-layout header="New Message">
    <div class="space-y-6">
        <div class="flex items-center gap-3">
            <a href="{{ route('teacher.messages.index') }}" class="w-9 h-9 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 hover:bg-slate-200 transition-colors">
            ...
            <div>
                ...
                <p class="text-xs text-slate-400 font-semibold mt-0.5">Send a message to a homeroom student's parent</p>
            </div>
        </div>

        <div x-data="{
            selectedRecipientId: '{{ old('recipient_id', $preselectedId ?? '') }}',
            recipients: @js($recipients),
            get selectedRecipient() {
                return this.recipients.find(r => r.guardian_user_id == this.selectedRecipientId) || null;
            },
            avatarColor(name) {
            ...
            <div class="lg:col-span-1">
                <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
                    <div class="h-20 bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-500"></div>
                    <div class="px-5 pb-5 relative">
                        <div class="absolute -top-8 left-5">
                            <template x-if="selectedRecipient">
                                <div class="w-16 h-16 rounded-2xl text-white flex items-center justify-center font-black text-xl border-4 border-white dark:border-slate-900 shadow-lg uppercase"
                                     :class="avatarColor(selectedRecipient.guardian_name)"
                                     x-text="selectedRecipient.guardian_name.charAt(0)"></div>
                            </template>
                            <template x-if="!selectedRecipient">
                            ...
                            <template x-if="selectedRecipient">
                                <div>
                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-indigo-50 text-indigo-600 mb-1">
                                        ...
                                        <span x-text="selectedRecipient.relationship"></span>
                                    </span>
                                    <h3 class="text-base font-black text-slate-800 dark:text-slate-100" x-text="selectedRecipient.guardian_name"></h3>
                                    <p class="text-xs text-slate-500 mt-1" x-text="'Parent of ' + selectedRecipient.student_name + ' (Section ' + '{{ $section->name }}' + ')'"></p>
                                </div>
                            </template>
                            <template x-if="!selectedRecipient">
                                <p class="text-sm text-slate-400 font-semibold">Select a parent to see details.</p>
                            </template>
                        </div>
                    </div>
                </div>
            ...
            <div class="lg:col-span-2">
                <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
                    ...
                    <form method="POST" action="{{ route('teacher.messages.store') }}" class="space-y-5">
                        @csrf
                        <div>
                            <label class="block text-xs font-black text-slate-500 uppercase tracking-widest mb-1.5">Select Parent <span class="text-rose-500">*</span></label>
                            <select name="recipient_id" x-model="selectedRecipientId"
                                ...
                                @foreach($recipients as $recipient)
                                    <option value="{{ $recipient['guardian_user_id'] }}" :selected="selectedRecipientId == {{ $recipient['guardian_user_id'] }}">
                                        {{ $recipient['guardian_name'] }} (Parent of {{ $recipient['student_name'] }} - {{ $recipient['relationship'] }})
                                    </option>
                                @endforeach
                            </select>
                            @error('recipient_id') <p class="text-xs text-rose-500 mt-1 font-semibold">{{ $message }}</p> @enderror
                        </div>
                        ...
                        <div class="flex justify-end">
                            <button type="submit"
                                ...
                                class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-bold rounded-xl shadow-md hover:shadow-lg transition-all text-sm disabled:opacity-40 disabled:cursor-not-allowed">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
