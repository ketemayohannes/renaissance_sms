<x-teacher-layout header="Conversation">
            ...
            <div class="border-t border-slate-100 dark:border-slate-800 p-4">
                <form @submit.prevent="submitReply()" class="flex gap-3">
                    <textarea x-model="replyBody" rows="2" placeholder="Write a reply..."
                        ...
                        class="flex-1 px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-slate-100 placeholder-slate-400 font-medium focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all resize-none" required></textarea>
                    <button type="submit" :disabled="loading" class="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl shadow transition-colors flex items-center gap-1.5 self-end text-sm disabled:opacity-50">
