<x-admin-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="card overflow-hidden">
            <div class="p-6">
                    ...
                    <form action="{{ route('admin.grade-components.store') }}" method="POST">
                        @csrf
                            ...
                            <div>
                                ...
                                <select name="academic_year_id" id="academic_year_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                    ...
                                    @foreach($academicYears as $year)
                                        <option value="{{ $year->id }}" {{ (old('academic_year_id') == $year->id || (!old('academic_year_id') && $year->is_active)) ? 'selected' : '' }}>{{ $year->name }}</option>
                                    @endforeach
                                </select>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function loadTerms(yearId, selectedTermId = null) {
                if(!yearId) {
                ...
                fetch(`{{ route('admin.gradebook.get-terms') }}?academic_year_id=${yearId}`)
                    ...
                    .then(data => {
                        termSelect.innerHTML = '<option value="">All Terms</option>';
                        data.forEach(term => {
                            const selected = selectedTermId == term.id ? 'selected' : '';
