<x-admin-layout>
    <div class="space-y-8 pb-12" x-data="{ staffCategory: '{{ old('staff_category', 'administrative') }}' }">
        <!-- Modern Header -->
        ...
        <form action="{{ route('admin.employees.store') }}" method="POST" enctype="multipart/form-data" class="space-y-12">
            ...
            <div class="bg-white/80 backdrop-blur-xl border border-white rounded-[3rem] shadow-2xl p-10 relative overflow-hidden group">
                ...
                <div class="relative z-10">
                    <div class="flex items-center gap-4 mb-10 border-b border-slate-50 pb-8">
                        ...
                        <div>
                            <h3 class="text-xl font-black text-slate-900 uppercase tracking-tight italic">Biological Identity</h3>
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Foundational demographics and personal classification设计</p>
                        </div>
                    </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                <div class="bg-white/80 backdrop-blur-xl border border-white rounded-[3rem] shadow-2xl p-10 group overflow-hidden">
                    ...
                    <div class="space-y-8">
                        <div class="space-y-2">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Digital Mail (Primary) <span class="text-rose-500">*</span></label>
                            <input type="email" name="email" value="{{ old('email') }}" required
                                   class="w-full bg-slate-50 border-slate-100 rounded-[1.4rem] py-4 px-6 focus:ring-emerald-600 focus:border-emerald-600 text-sm font-black shadow-inner lowercase italic">
                        </div>
                        <div class="space-y-2">
                ...
                <div class="bg-white/80 backdrop-blur-xl border border-white rounded-[3rem] shadow-2xl p-10 group overflow-hidden">
                    ...
                    <div class="grid grid-cols-2 gap-8">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="woreda" value="{{ old('woreda') }}"
                                   class="w-full bg-slate-50 border-slate-100 rounded-[1.4rem] py-4 px-6 focus:ring-rose-600 focus:border-rose-600 text-sm font-black shadow-inner">
                        </div>
                        <div class="space-y-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="department" class="w-full bg-slate-50 border-slate-100 rounded-[1.4rem] py-4 px-6 focus:ring-amber-600 focus:border-amber-600 text-sm font-black appearance-none shadow-inner">
                            ...
                            <option value="Administration">INSTITUTIONAL ADMIN</option>
                            <option value="Finance">FINANCIAL OPS</option>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="employment_type" required class="w-full bg-slate-50 border-slate-100 rounded-[1.4rem] py-4 px-6 focus:ring-amber-600 focus:border-amber-600 text-sm font-black appearance-none shadow-inner">
                            ...
                            <option value="part_time">PART TIME / TACTICAL</option>
                            <option value="contract">SPECIFIC CONTRACT</option>
                        </select>
                    </div>
                ...
                <div class="relative z-10">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="teacher_rank" value="{{ old('teacher_rank') }}"
                                   class="w-full bg-white/5 border-white/10 rounded-[1.4rem] py-4 px-6 focus:ring-white focus:border-white text-sm font-black text-white shadow-xl placeholder:text-white/20 uppercase italic" placeholder="e.g. SENIOR FACULTY">
                        </div>
                        <div class="space-y-2">
                            ...
                            <select name="qualification_level" class="w-full bg-white/5 border-white/10 rounded-[1.4rem] py-4 px-6 focus:ring-white focus:border-white text-sm font-black text-white appearance-none shadow-xl">
                                ...
                                <option value="Diploma" class="bg-indigo-950">DIPLOMA LEVEL</option>
                                <option value="Degree" class="bg-indigo-950">BACHELOR DEGREE</option>
                                <option value="Masters" class="bg-indigo-950">MASTER OF SCIENCE/ARTS</option>
                                <option value="PhD" class="bg-indigo-950">DOCTORATE / PhD</option>
                            </select>
                        </div>
                        <div class="space-y-2 md:col-span-1">
                            <label class="text-[10px] font-black text-indigo-400 uppercase tracking-widest ml-1">Subject Major</label>
                            <input type="text" name="specialization" value="{{ old('specialization') }}"
                                   class="w-full bg-white/5 border-white/10 rounded-[1.4rem] py-4 px-6 focus:ring-white focus:border-white text-sm font-black text-white shadow-xl placeholder:text-white/20 uppercase italic" placeholder="e.g. QUANTITATIVE ANALYSIS">
                        </div>
                        <div class="space-y-2">
                            <label class="text-[10px] font-black text-indigo-400 uppercase tracking-widest ml-1">Periods / Cycle</label>
                            <input type="number" name="periods_per_week" value="{{ old('periods_per_week') }}"
                                   class="w-full bg-white/5 border-white/10 rounded-[1.4rem] py-4 px-6 focus:ring-white focus:border-white text-sm font-black text-white shadow-xl placeholder:text-white/20" placeholder="0">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col md:flex-row items-center justify-between gap-8 pt-10">
                <div class="flex items-center gap-6 bg-slate-50/50 p-4 pr-10 rounded-[2rem] border border-slate-100">
                    <div class="w-16 h-16 rounded-[1.5rem] bg-slate-900 flex items-center justify-center text-white shadow-2xl">
                        <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path></svg>
                    </div>
                    <div>
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Data Governance</p>
                        <p class="text-sm font-bold text-slate-600 italic">Personnel entries are audit-tracked and encrypted within the secure vault.</p>
                    </div>
                </div>

                <div class="flex items-center gap-4 w-full md:w-auto">
                    <a href="{{ route('admin.employees.index') }}" class="flex-1 md:flex-none py-5 px-10 bg-white border border-slate-200 text-slate-400 font-black text-[10px] uppercase tracking-widest rounded-3xl hover:bg-rose-50 hover:text-rose-600 transition-all active:scale-95 text-center">
                        Abort Registry
                    </a>
