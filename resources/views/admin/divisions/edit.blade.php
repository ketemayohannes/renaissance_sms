<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between w-full relative z-10">
            <h2 class="text-2xl font-black text-slate-800 uppercase tracking-tight">Modify Division: {{ $division->name }}</h2>
        </div>
    </x-slot>
    ...
    <div class="space-y-8 pb-12">
        <x-breadcrumb :items="[
            ...
            ['label' => 'Modification Portal', 'url' => '#']
        ]" />

        <div class="max-w-3xl mx-auto">
            <div class="glass-panel overflow-hidden border-white/40 shadow-2xl">
                <div class="p-8 border-b border-slate-50 bg-slate-50/30">
                    <h3 class="text-sm font-black text-slate-900 uppercase tracking-widest">Division Revision</h3>
                    <p class="text-xs text-slate-500 font-medium mt-1 italic">Update the parameters for this academic division.</p>
                </div>

                <div class="p-8">
                    <form action="{{ route('admin.divisions.update', $division) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea name="description" id="description" rows="3" 
                                class="premium-input w-full">{{ old('description', $division->description) }}</textarea>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                <label for="sort_order" class="px-1 text-[10px] font-black text-slate-400 uppercase tracking-widest">Sequence Order *</label>
                                <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $division->sort_order) }}" 
                                    class="premium-input w-full" required>
                            </div>
