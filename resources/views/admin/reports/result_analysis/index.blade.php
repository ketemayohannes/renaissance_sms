    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white/60 backdrop-blur-xl rounded-[2.5rem] border border-white shadow-2xl shadow-slate-200/50 p-10">
                <div class="mb-10">
                    <h3 class="text-2xl font-black text-slate-900 font-heading uppercase tracking-tight">View Result Analysis</h3>
                    <p class="text-[11px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-1">Select a teacher's class and term to view their result analysis report.</p>
                </div>

                <form action="" id="selectionForm" class="space-y-8">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                        <div class="space-y-2">
                            <label for="assignment_id" class="block text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-1">Teacher / Class / Subject</label>
    ...
    <script>
        function goToReport() {
            ...
            if (!assignmentId || !termId) {
                alert('Please select both a report and a term.');
                return;
            }

            window.location.href = `{{ url('admin/result-analysis') }}/${assignmentId}?term_id=${termId}`;
        }
    </script>
</x-admin-layout>
