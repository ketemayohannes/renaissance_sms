<x-admin-layout>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const STORAGE_KEY = 'section_grades_selections';
            ...
            function loadSections(yearId, gradeId, savedSectionId) {
                return fetch(`{{ route('admin.gradebook.get-sections') }}?academic_year_id=${yearId}&grade_level_id=${gradeId}`)
                    .then(response => response.json())
                    .then(data => {
            ...
            if (saved) {
                try {
                    ...
                    if (s.term_id) termSelect.value = s.term_id;
                    if (s.grade_level_id) {
                        gradeSelect.value = s.grade_level_id;
                        // Trigger AJAX to load sections, then set saved value
                        if (yearSelect.value) {
                            loadSections(yearSelect.value, s.grade_level_id, s.section_id);
                        }
                    }
                } catch (e) {
