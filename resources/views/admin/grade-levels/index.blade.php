<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between w-full relative z-10">
            <h2 class="text-2xl font-black text-slate-800 uppercase tracking-tight">Grade Architecture</h2>
            <a href="{{ route('admin.grade-levels.create') }}" class="vibrant-btn-blue">
    ...
    <div class="space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-30">
            <x-breadcrumb :items="[['label' => 'Grade Levels', 'url' => '#']]" />
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/50 border-b border-slate-100">
                            <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Level Identity</th>
                            <th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Short Code</th>
                            <th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Division</th>
                            ...
                            <tr class="hover:bg-indigo-50/30 transition-all group">
                                <td class="px-8 py-6">
                                    <div class="flex items-center gap-4">
                                        <div class="w-10 h-10 rounded-xl bg-violet-500/10 flex items-center justify-center text-violet-600 font-black text-sm group-hover:rotate-6 transition-transform">
                                            {{ substr($gradeLevel->name, 0, 1) }}
                                        </div>
                                        <span class="font-bold text-slate-900 tracking-tight">{{ $gradeLevel->name }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-6">
                                    <span class="px-3 py-1 bg-slate-100 text-slate-600 rounded-lg text-xs font-black uppercase tracking-widest">{{ $gradeLevel->code }}</span>
                                </td>
                                <td class="px-6 py-6 font-semibold text-slate-600 text-sm">
                                    <div class="flex items-center gap-2">
                                        <span class="w-1.5 h-1.5 rounded-full bg-indigo-400"></span>
                                        {{ $gradeLevel->division->name }}
                                    </div>
                                </td>
                                <td class="px-6 py-6 text-slate-400 font-bold text-xs">{{ $gradeLevel->sort_order }}</td>
                                <td class="px-6 py-6">
                                ...
                                <td class="px-8 py-6 text-right">
                                    <div class="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <a href="{{ route('admin.grade-levels.edit', $gradeLevel) }}" class="p-2 bg-white border border-slate-200 rounded-lg text-indigo-600 hover:bg-indigo-600 hover:text-white hover:border-indigo-600 transition-all shadow-sm">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        </a>
                                        <form action="{{ route('admin.grade-levels.destroy', $gradeLevel) }}" method="POST" class="inline-block delete-form" data-confirm-message="Are you sure you want to delete this grade level?">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <tr>
                                <td colspan="6" class="px-8 py-12 text-center">
                                    <div class="flex flex-col items-center gap-4 opacity-40">
                                        ...
                                        <p class="text-sm font-black uppercase tracking-widest text-slate-500 italic">Static Environment: No Grade Levels Found</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
