<x-admin-layout>
    ...
    <div class="space-y-8 pb-20">
        ...
        <form action="{{ route('admin.disciplinary.store') }}" method="POST"
              x-data="{
                  selectedInfractionId: '{{ old('infraction_definition_id') }}',
                  infractions: {{ $infractionDefinitions->map(fn($d) => ['id' => $d->id, 'name' => $d->name, 'tier' => $d->tier, 'display_tier' => $d->display_tier, 'tier_color' => $d->tier_color, 'default_penalty' => $d->default_penalty, 'requires_parent_notification' => $d->requires_parent_notification])->values()->toJson() }},
                  get selected() {
                      return this.infractions.find(i => i.id === this.selectedInfractionId) || null;
                  }
              }">
            @csrf
                ...
                <div class="space-y-8">
                    <div class="bg-white/80 backdrop-blur-xl rounded-[2.5rem] border border-white shadow-2xl shadow-slate-200/50 p-8">
                        ...
                        <div class="space-y-6">
                            <!-- Infraction Type -->
                            <div class="space-y-2">
                                <label class="px-1 text-[9px] font-black text-slate-400 uppercase tracking-widest">Infraction Type</label>
                                <select name="infraction_definition_id" class="premium-select w-full" required
                                        x-model="selectedInfractionId">
                                    <option value="">Select Infraction Type</option>
                                    @foreach($infractionDefinitions->groupBy('tier') as $tier => $group)
                                        <optgroup label="{{ ucfirst($tier) }} Infractions">
                                            @foreach($group as $def)
                                                <option value="{{ $def->id }}" {{ old('infraction_definition_id') == $def->id ? 'selected' : '' }}>
                                                    {{ $def->name }}
                                                </option>
                                            @endforeach
                                        </optgroup>
                            ...
                            <!-- Dynamic Tier Badge -->
                            <div x-show="selected" x-transition class="space-y-2">
                                <div class="flex items-center justify-between p-4 rounded-2xl border"
                                     :class="{
                                        'bg-emerald-50 border-emerald-100': selected?.tier === 'minor',
                                        'bg-amber-50 border-amber-100': selected?.tier === 'moderate',
                                        'bg-rose-50 border-rose-100': selected?.tier === 'critical'
                                     }">
                                    <div>
                                        <span class="block text-[9px] font-black uppercase tracking-widest"
                                              :class="{
                                                'text-emerald-600': selected?.tier === 'minor',
                                                'text-amber-600': selected?.tier === 'moderate',
                                                'text-rose-600': selected?.tier === 'critical'
                                              }">Severity Tier</span>
                                        <span class="block text-sm font-black mt-1"
                                              :class="{
                                                'text-emerald-700': selected?.tier === 'minor',
                                                'text-amber-700': selected?.tier === 'moderate',
                                                'text-rose-700': selected?.tier === 'critical'
                                              }" x-text="selected?.display_tier"></span>
                                    </div>
                                    <div class="text-right">
                                        <span class="block text-[9px] font-black text-slate-400 uppercase tracking-widest">Default Penalty</span>
                                        <span class="block text-xs font-semibold text-slate-600 mt-1" x-text="selected?.default_penalty || '—'"></span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <label class="flex items-start gap-4 p-4 rounded-2xl bg-rose-50/50 border border-rose-100/50 cursor-pointer group">
                                <div class="relative flex items-center mt-1">
                                    <input type="checkbox" name="notify_parent" value="1"
                                           :checked="selected?.requires_parent_notification"
                                ...
                                <div>
                                    ...
                                    <span class="block text-[9px] font-semibold text-rose-600/70 uppercase mt-0.5"
                                          x-text="selected?.requires_parent_notification ? 'Required by infraction policy' : 'Automated notification dispatch'"></span>
                                </div>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="p-8 bg-slate-900 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden group">
                        <div class="absolute -right-10 -bottom-10 w-40 h-40 bg-indigo-600/20 rounded-full blur-3xl group-hover:scale-150 transition-transform duration-700"></div>
                        <div class="relative z-10">
                            ...
                            <p class="text-[10px] font-medium text-slate-400 leading-relaxed uppercase tracking-wider">The system will automatically evaluate escalation rules when this record is saved and may apply additional actions based on prior incident history.</p>
                        </div>
                    </div>
                </div>
            </div>
